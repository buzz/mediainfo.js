...
import type { MediaInfoType } from '../src/MediaInfoType'
...
describe('coverData: Dead_Combo_-_01_-_Povo_Que_Cas_Descalo.mp3', () => {
  const testFields = (result: MediaInfoType) => {
    ...
    expect(track[0].OverallBitRate).toBeNear(243043, 2)
    expect(track[0].StreamSize).toBe(175116)
    ...
    expect(track[1].BitRate).toBeNear(243043, 2)
    expect(track[1].BitRate_Minimum).toBeCloseTo(32000.0)
    expect(track[1].Channels).toBe(2)
    expect(track[1].SamplesPerFrame).toBeCloseTo(1152.0)
    expect(track[1].SamplingRate).toBe(44100.0)
    expect(track[1].SamplingCount).toBe(8974080)
    expect(track[1].FrameRate).toBeCloseTo(38.281)
    expect(track[1].Compression_Mode).toBe('Lossy')
    expect(track[1].StreamSize).toBe(6182244)
